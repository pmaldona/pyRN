...
class RNIRG:
    ...
    def display(self,p=np.array([])):
        ...
        #Function that displays stoichiometric matrix
    def display_stoich(self):
        Sx=self.mp-self.mr
        max_xval = Sx.shape[0]
        max_yval = Sx.shape[1]
        #print("Numero especies = "+str(Sx.shape[0]))
        #print("Numero reacciones = "+str(Sx.shape[1]))
        Sx_np=Sx.to_numpy()
        fig, ax = plt.subplots()
        ax.matshow(Sx_np, cmap=plt.cm.viridis)
        for i in range(max_yval):
            for j in range(max_xval):
                c = Sx_np[j,i]               
