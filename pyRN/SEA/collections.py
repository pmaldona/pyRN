...
def get_combinations(lst: list)->list:
    '''
    Parameters:
        lst (list),
    Returns: combinations_list (list), a list with all the combinations of the elements from lst as list
    '''
    combinations_list = []
...
def conditional_probability_estimator_3(abstractions_df, transitions_df, organization_indices):
    ...
    if s:
        ...
        return conditional_probability_estimator_2(abstractions_df, transitions_df, organization_indices)
...
def cooperative(abstractions_df, transitions_df, organization_indices):
    '''
    try:
        a = local_collective_resilience(abstractions_df, transitions_df, organization_indices, estimator=3)
    except:
        a = 0
        print(organization_indices)
    '''
    a = local_collective_resilience(abstractions_df, transitions_df, organization_indices, estimator=3)
    b = max([gr for i, gr in enumerate(abstractions_df['local_resilience'].tolist()) if i in organization_indices])
    return a > b*1.01

def find_cooperative_sets(abstractions_df, transitions_df, strongly_connected=True):
    '''
    Parameters:
        abstractions_df,
        transitions_df,
        strongly_connected
    Returns:
        A dataframe with keys 'set' and 'local_collective_resilience
    '''
    ...
    sets            = [s for s in sets if cooperative(abstractions_df, transitions_df, s)]
    resiliences     = [local_collective_resilience(abstractions_df, transitions_df, organization_indices, 3) for organization_indices in sets]
    return {'set': sets, 'local collective resilience': resiliences}